...
if (!isset($_GET['album'])) {
    header("Location: songs-error.php?code=1");
    exit();
}
$album = $_GET['album'];
...
if (sizeof($row) == 0) {
    header("Location: songs-error.php?code=2");
    exit();
}
...
<div class="container containerSongs">
    ...
    <?php
    if (sizeof($songName) == 0) {
        echo "<div class='row justify-content-center text-center'>
                  <h5 class='col-md-6'> Sorry, but there are no songs in our database for this album yet </h5>
              </div>";
    } else
